...
class KWZParser:
  ...
  def get_audio_track(self, track_index):
    if not hasattr(self, "track_lengths"):
      raise ValueError("No KSN section in this kwz file")
    ...
    for byte in self.buffer.read(size):
      ...
      while bit_pos < 8:
        ...
        if prev_step_index < 18 or bit_pos > 4:
          ...
          # get diff
          diff = int(prev_diff) + int(ADPCM_SAMPLE_TABLE_2[sample + 4 * prev_step_index])
          # get step index
          step_index = prev_step_index + int(ADPCM_INDEX_TABLE_2[sample])
          bit_pos += 2
        ...
        # else read a 4-bit sample
          ...
          # get diff
          diff = int(prev_diff) + int(ADPCM_SAMPLE_TABLE_4[sample + 16 * prev_step_index])
          # get step index
          step_index = prev_step_index + int(ADPCM_INDEX_TABLE_4[sample])
          bit_pos += 4

        # clamp step index (diffは最終PCMでクリップする)
